<?php
	...
	if(isset($_POST['itemNumber'])) {
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($itemDetailsQuantity) && isset($itemDetailsUnitPrice)){
			...
			$updateItemDetailsSql = 'UPDATE item SET itemName = :itemName,  stock = :stock, unitPrice = :unitPrice, status = :status, WHERE itemNumber = :itemNumber';
			$updateItemDetailsStatement = $conn->prepare($updateItemDetailsSql);
			$updateItemDetailsStatement->execute(['itemName' => $itemName, 'stock' => $newStock, 'unitPrice' => $itemDetailsUnitPrice, 'status' => $status, 'itemNumber' => $itemNumber]);
